...
Section ADEQ.
  ...
  Theorem local_adequacy
          ...
          (* tidl tidr *)
          (LOCAL: forall tid (tgt: itree tgtE R1) (LTGT: threads_find tid ths_tgt = Some tgt),
            exists (src: itree srcE R0), (threads_find tid ths_src = Some src) /\
                                      (local_sim world_le I RR src tgt))
          (* (LOCAL: forall tidr0 (tgt: itree tgtE R1) (LTGT: threads_find tidr0 (tidr :: ths_tgt) = Some tgt), *)
          (*   exists tidl0 (src: itree srcE R0), (threads_find tidl0 (tidl :: ths_src) = Some src) /\ *)
          (*                                   (local_sim world_le I RR src tgt)) *)
          (* (THSRC: threads_find tidl ths_src = None) *)
          (* (THTGT: threads_find tidr ths_tgt = None) *)
          tidl tidr src tgt
          (SRCIN: threads_find tidl ths_src = Some src)
          (TGTIN: threads_find tidr ths_tgt = Some tgt)
  ...
  Proof.
    ...
    hexploit LOCAL; eauto. i; des. rename H into FIND, H0 into LSIM.
    unfold local_sim in LSIM.
    hexploit LSIM; clear LSIM. 3: eauto.
    { instantiate (1:=tidr). admit. }
    { admit. }
    ...
    match goal with
    | LSIM: lsim _ _ ?_LRR tidr _ _ _ _ ?_shr |- _ => remember _LRR as LRR; remember _shr as shr
    end.
    ...
    { instantiate (1:= (fun gps gpt (src0: itree srcE R0) (tgt: itree tgtE R1) shr =>
                          forall (tidl : nat) (src : thread (sE state_src)),
                            threads_find tidl ths_src = Some src ->
                            threads_find tidr ths_tgt = Some tgt ->
                            forall (st_src : state_src) (st_tgt : state_tgt) (mt : imap wf_tgt) (ms : imap wf_src) (w : world),
                              threads_find tidr ths_src = Some src0 ->
                              LRR = local_RR world_le I RR tidr ->
                              shr = (alist_proj1 ths_src, alist_proj1 ths_tgt, ms, mt, st_src, st_tgt, o, w) ->
                              gpaco9 (_sim (wft:=wf_tgt)) (cpn9 (_sim (wft:=wf_tgt))) bot9 r R0 R1 RR gps ms gpt mt (interp_all st_src ths_src tidl src)
                                     (interp_all st_tgt ths_tgt tidr tgt))) in LSIM. auto. }

    ss. clear gps gpt src0 tgt shr LSIM.
    intros rr DEC IH ps pt src0 tgt shr LSIM. clear DEC.
    intros tidl src THSRC THTGT st_src st_tgt mt ms w FIND ELRR Eshr.
    eapply pind5_unfold in LSIM.
    2:{ eapply _lsim_mon. }
    inv LSIM.

    { clarify. clear IH rr. unfold local_RR in LSIM0. des.
