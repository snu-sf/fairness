...
From Fairness Require Import
     IProp IPM SimWeakest ModSim PCM MonotoneRA FairnessRA
     TemporalLogic OneShotsRA AuthExclsRA ExclsRA.
From Fairness Require NatStructs NatMapRA.
...
Section SIM.
  ...
  Context `{sub : @SRA.subG Γ Σ}
          ...
          {AUTHNMNRA: @GRA.inG (Auth.t (NatMapRA.NatMapRA.t nat)) Γ}
  ...
  Definition natmap_sprop_sum {n} A (f: NatStructs.NatMap.t A) (P: nat -> A -> sProp n) :=
    list_sprop_sum (NatStructs.NatMap.elements f) (fun '(k, v) => P k v).

  Lemma red_sprop_sum n A (f: NatStructs.NatMap.t A) (P: nat -> A -> sProp n) :
    ⟦natmap_sprop_sum f P, n⟧ = NatMapRA.natmap_prop_sum f (fun t a => ⟦P t a, n⟧).
  Proof.
    unfold natmap_sprop_sum, NatMapRA.natmap_prop_sum.
    induction (NatStructs.NatMap.elements _); ss; red_tl.
    rewrite IHl. des_ifs.
  Qed.
  ...
  Lemma natmap_prop_sum_pull_fupd
    ...
    (NatMapRA.natmap_prop_sum m (fun k a => =| x |=( Q )={E}=> P k a))
      -∗
      =| x |=( Q )={E, E}=>(NatMapRA.natmap_prop_sum m P).
  Proof.
    unfold NatMapRA.natmap_prop_sum. iIntros "SUM". iStopProof.
    generalize (NatStructs.NatMap.elements m) as l.
    induction l.
    { iIntros "_". ss. }
  ...
  Definition lockInv i γtv γsv γl γing : sProp i :=
    (∃ (svw: τ{View.t, i}) (waits: τ{NatStructs.NatMap.t nat, i})
        (mem: τ{WMem.t, i}) (own ing : τ{bool, i}) (κu γs: τ{nat, i}),
      ➢ (Auth.black (Some waits: NatMapRA.NatMapRA.t nat))
      ∗ STTGT {(tt, (mem, (((own, svw), ing), NatStructs.key_set waits)))}
      ∗ s_wmemory_black mem
      ∗ ● γsv (κu, γs, svw)
      ...
      ∗ ⟨Atom.fair_blacks_tgt (inrp ⋅ inrp ⋅ inrp)%prism (fun id => (~ NatStructs.NatMap.In id waits))⟩
      ∗ ((⌜own = false⌝ ∗ ○ γtv svw ∗ ○ γsv (κu, γs, svw) ∗ EX γl tt)
  ...
  Proof.
    ...
    iMod (pcs_decr _ _ 1 1 with "PCS") as "[PCS1 PCS2]". auto.
    
    iMod (pcs_drop _ _ _ _ 2 2 with "PCS2") as "PCS2". Unshelve. all: auto.
    iMod (pcs_decr _ _ 1 1  with "PCS2") as "[PCS2 PCS3]". auto.
    iMod (pcs_drop _ _ _ _ 1 2 with "PCS3") as "PCS3". Unshelve. all: auto.
    ...
    erewrite <- (NatStructs.key_set_pull_add_eq _ κa).
    remember (NatStructs.NatMap.add tid _ _) as waits'.

    iAssert (⌜~ NatStructs.NatMap.In tid waits⌝)%I as "%".
    { iAssert (⌜(NatStructs.NatMap.In tid waits)⌝ ∨ ⌜(~ NatStructs.NatMap.In tid waits)⌝)%I as "%".
      { iPureIntro. pose NatStructs.NatMapP.F.In_dec. specialize (s _ waits tid). destruct s; auto. }
      destruct H as [IN | NI].
      2: auto.
      ...
      apply NatStructs.NatMapP.F.in_find_iff in IN.
      destruct (NatStructs.NatMap.find tid waits) eqn:FIND; ss.
    ...
    set (blks2 := fun id => ¬ NatStructs.NatMap.In id (waits')%type).
    iPoseProof (FairRA.blacks_unfold with "LI6") as "[LI6 MYDUTY]".
    { instantiate (1:=tid). instantiate (1:=blks2). i. des.
      { subst blks2 waits'. ii. apply IN. apply NatStructs.NatMapP.F.add_in_iff; auto. }
      { subst blks2. subst. auto. }
    }
    { subst blks2 waits'. ss. ii. eapply H0. apply NatStructs.NatMapP.F.add_in_iff. auto. }
    ...
    { eapply Auth.auth_alloc. instantiate (1:=NatMapRA.NatMapRA.singleton tid κa).
      instantiate (1:=Some (NatStructs.NatMap.add tid κa waits)). eapply NatMapRA.NatMapRA.add_local_update.
      eapply NatStructs.NatMapP.F.not_find_in_iff; auto.
    }
    ...
    iMod (pcs_drop _ _ _ _ 1 4 with "PCS") as "PCS". Unshelve. all: auto.
    ...
    { iClear "IH".
      ...
      iAssert (⌜NatStructs.NatMap.find tid waits = Some κa⌝)%I as "%FIND".
      { iPoseProof (OwnM_valid with "[MY LI1]") as "%".
        { instantiate
            (1:= (Auth.black (Some waits: NatMapRA.NatMapRA.t nat))
            ⋅ (Auth.white (NatMapRA.NatMapRA.singleton tid κa: NatMapRA.NatMapRA.t nat))).
          iSplitL "LI1"; iFrame.
        }
        eapply Auth.auth_included in H. eapply NatMapRA.NatMapRA.extends_singleton_iff in H.
        auto.
      }

      iPoseProof (NatMapRA.natmap_prop_remove_find with "LI5") as "[LIMY LI5]". apply FIND.
      remember (NatStructs.NatMap.remove _ _) as waits'.
      ...
      { rewrite <- NatStructs.key_set_pull_rm_eq. rewrite <- Heqwaits'.
        instantiate (1:=List.map fst (NatStructs.NatMap.elements (NatStructs.key_set waits'))).
        subst waits'. clear. i.
        assert (A: NatStructs.NatMap.In i (NatStructs.key_set (NatStructs.NatMap.remove tid waits))).
        { apply in_map_iff in IN. des. des_ifs. destruct x. destruct u.
          remember (NatStructs.NatMap.remove tid (NatStructs.key_set waits)) as M. clear HeqM.
          apply NatStructs.NatMapP.F.elements_in_iff. exists (). apply SetoidList.InA_alt.
          exists (k, ()). ss.
        }
        des_ifs. exfalso. eapply NatStructs.NatMap.remove_1. reflexivity.
        rewrite NatStructs.key_set_pull_rm_eq in A. apply A.
      }
      { eapply FinFun.Injective_map_NoDup.
        { unfold FinFun.Injective. i. des_ifs. destruct x, y. destruct u, u0. ss. subst. auto. }
        apply NoDupA_NoDup. apply NatStructs.NatMap.elements_3w.
      }
      iIntros "MY3 WHITES". rred2r.

      iPoseProof (OwnM_Upd with "[LI1 MY]") as "> LI1".
      2:{ instantiate
          (1:= (Auth.black (Some waits: NatMapRA.NatMapRA.t nat))
            ⋅ (Auth.white (NatMapRA.NatMapRA.singleton tid κa: NatMapRA.NatMapRA.t nat))). iSplitL "LI1"; iFrame.
      }
      { eapply Auth.auth_dealloc. eapply NatMapRA.NatMapRA.remove_local_update. }
      rewrite <- Heqwaits'.
      ...
      { instantiate (1:=(fun id => ¬ NatStructs.NatMap.In id waits')).
        i. ss. des. destruct (tid_dec j tid) eqn:DEC.
        - clarify. auto.
        - left. esplits; eauto. ii. apply IN. subst. apply NatStructs.NatMapP.F.remove_in_iff. split; auto.
      }
      ...
      { Transparent NatStructs.key_set. unfold NatStructs.key_set. rewrite <- NatStructs.list_map_elements_nm_map.
        unfold NatMapRA.natmap_prop_sum.
        remember (NatStructs.NatMap.elements waits') as ml. clear Heqml. rewrite List.map_map.
      ...
      { iEval (unfold lockInv).
        ...
        iFrame. rewrite <- NatStructs.key_set_pull_rm_eq. rewrite <- Heqwaits'. iFrame.
      ...
      iMod (pcs_decr _ _ 1 with "PCS") as "[PCS1 PC2]". left.
    ...
    { iApply (wpsim_getR). iSplit. auto. rred2r.
      ...
      iAssert (⌜NatStructs.NatMap.find tid waits = Some κa⌝)%I as "%FIND".
      { iPoseProof (OwnM_valid with "[LI1 MY]") as "%".
        { instantiate
          (1:= (Auth.black (Some waits: NatMapRA.NatMapRA.t nat))
          ⋅ (Auth.white (NatMapRA.NatMapRA.singleton tid κa: NatMapRA.NatMapRA.t nat))). iSplitL "LI1"; iFrame. }
        eapply Auth.auth_included in H. eapply NatMapRA.NatMapRA.extends_singleton_iff in H.
  ...
  Section SIM_INIT.
    ...
    :
    (OwnM (wmem_init_res loc_X (Loc.of_nat 210)))
      ∗ ((AuthExcls.rest (gt_dec 0) (0, 0, View.bot))
        ∗ (AuthExcls.rest (gt_dec 0) View.bot)
        ∗ (OwnM (Auth.black (Some (NatStructs.NatMap.empty nat): NatMapRA.NatMapRA.t nat)))
    ...
    Proof.
      ...
      { simpl. unfold lockInv.
        red_tl. iExists View.bot. red_tl. iExists (NatStructs.NatMap.empty nat).
        red_tl. iExists (WMem.init). red_tl. iExists false. red_tl. iExists false.
        red_tl. iExists 0. red_tl. iExists 0. red_tl_all. simpl.
        unfold OMod.closed_st_init, Mod.st_init; ss. rewrite NatStructs.key_set_empty_empty_eq.
