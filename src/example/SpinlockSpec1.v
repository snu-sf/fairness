...
Section SPEC.
  ...
  (** Invariants. *)
  Definition spinlockInv (n : nat) (x : SCMem.val) (γx : nat) (S : sProp n)
    : sProp n :=
    (∃ (s : τ{nat}), (x ↦ s) ∗ (● γx s) ∗ ((⌜s = 0⌝ ∗ S) ∨ (⌜s = 1⌝)))%S.
  ...
  Notation E_Spinlock1 := (↑N_Spinlock1 : coPset).

  Definition isSpinlock n (x : SCMem.val) (γx : nat) (S : sProp n) (k L : nat)
    : sProp n :=
    (∃ (N : τ{namespace, n}),
        (⌜(↑N ⊆ (↑N_Spinlock1 : coPset))⌝)
          ∗ ◆[k, L] ∗ syn_inv n N (spinlockInv n x γx S))%S.

  Global Instance isSpinlock_persistent n x γx S k L :
    Persistent (⟦isSpinlock n x γx S k L, n⟧).
  Proof.
  ...
  Lemma isSpinlock_get_data n x γx (S : sProp n) k L :
    (⟦isSpinlock n x γx S k L, n⟧) -∗ (◆[k, L] ∗ ∃ N, (⌜↑N ⊆ E_Spinlock1⌝) ∗ inv n N (spinlockInv n x γx S)).
  Proof.
    iEval (unfold isSpinlock; red_tl). iIntros "[%N ISL]".
    iEval (red_tl) in "ISL". iDestruct "ISL" as "[NS [LO INV]]". eauto.
  Qed.
  ...
  Lemma Spinlock_lock_spec
        ...
        (E : coPset)
    :
    ⊢ ∀ x γx (S : sProp n) k L v (ds : list (nat * nat * sProp n)) (P R Q : sProp n),
        [@ tid, n, E @]
          ⧼(tgt_interp_as (state_tgt:=st_tgt_type) sndl (fun m => ((s_memory_black m) : sProp n)%S))
             ∗ (⟦isSpinlock n x γx S k L, n⟧)
             ∗ (⟦P, n⟧)
             ∗ (⟦Duty(tid) ds, n⟧ ∗ ◇{List.map fst ds}(2 + L, 1))
             ∗ (env_live_inv v (1+n) (⊤ ∖ E_Spinlock1) k
                             (((x ↦ 1) ∗ (● γx 1))%S : sProp n)
                             ((x ↦ 0) ∗ (● γx 0) ∗ S)%S
               )
             ∗ (⟦((● γx 0) ∗ R)%S, n⟧ =|1+n|={⊤ ∖ E_Spinlock1}=∗ ⟦((● γx 1) ∗ Q)%S, n⟧)
  ...
  Proof.
    ...
    (* Preprocess for induction: right after the call's yield. *)
    iEval (rewrite unfold_iter_eq). rred2r.
    iDestruct "PRE" as "(#MEM & #ISL & P & [DUTY PCS] & #ELI & AU & #PUT_D & #GET_D)".
    iMod (ccs_make _ _ _ 1 (100+v) with "[PCS]") as "[CCS PCS]".
    { iFrame. iEval (unfold isSpinlock; red_tl) in "ISL". iDestruct "ISL" as "[%N ISL]".
      iEval (red_tl) in "ISL". iDestruct "ISL" as "[_ [LO _]]". eauto.
    }
    ...
    iEval (simpl) in "PCS".
    iPoseProof (isSpinlock_get_data with "ISL") as "[_ (% & %NS & #INV_SL)]".
    iInv "INV_SL" as "SL" "INV_SL_CLOSE". iEval (simpl) in "SL".
    iEval (unfold spinlockInv; red_tl_all) in "SL".
    iDestruct "SL" as "[%s SL]". iEval (red_tl_all) in "SL".
    iDestruct "SL" as "(PTx & Lx & [(%UNLOCKED & S) | %LOCKED])"; cycle 1.

    { (* Do induction. *)
      subst. iRevert "PCS INV_SL_CLOSE R AU POST".
      iPoseProof (eli_ccs_ind with "[CCS PCS_ELI] [] [] [PTx Lx]") as "RES".
      5:{ iSplitL; cycle 1. eauto. iEval (simpl; red_tl_all). iFrame. }
      5:{ iIntros "PCS INV_SL_CLOSE R AU POST". iMod "RES".
          iRevert "INV_SL_CLOSE R AU POST". iApply ("RES" with "PCS").
      }
      lia.
      { iFrame. }
      ...
      - (* Inductive case. *)
        ...
        iIntros "A". iEval (simpl; red_tl_all) in "A". iDestruct "A" as "[PTx Lx]".
        iModIntro. iIntros "PCS INV_SL_CLOSE R AU POST".
        ...
        lia.
        { assert (↑N_Spinlock1 ## (↑N_state_tgt : coPset)).
        ...
        { iEval (simpl; unfold spinlockInv; red_tl_all). iExists 1. iEval red_tl_all.
          iFrame. iRight. done.
        }
        ...
        iDestruct "SL" as "(PTx & Lx & [(%UNLOCKED & S) | %LOCKED])"; cycle 1.
        { (* Finish with IH. *)
          subst. iMod ("IH" with "FC [PTx Lx]") as "IH".
          { iEval (simpl; red_tl_all). iFrame. }
          iApply ("IH" with "INV_SL_CLOSE R AU POST").
        }
        ...
        iMod (FUpd_mask_mono _ _ _ (⊤ ∖ ↑N) with "AU") as "AU".
        { set_solver. }
        ...
        iApply "POST". iEval (red_tl). iFrame.

      - (* Terminating case. *)
        iIntros "T". iEval (simpl; red_tl_all) in "T". iDestruct "T" as "(PTx & Lx & S)".
        iModIntro. iIntros "INV_SL_CLOSE R AU POST".
        iApply (SCMem_cas_fun_spec with "[PTx] [-]").
        3:{ iFrame. iApply (tgt_interp_as_equiv with "MEM"). ss. i.
            red_tl_all. iSplit; [iIntros "A" | iIntros "[A M]"]; iFrame. ss.
        }
