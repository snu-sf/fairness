...
Section SIM.
  ...
  Lemma ClientSpinlock2_thread1_sim
        ...
        (LAYER_l : ℓl = 2)
    :
    ⊢ ⟦(∀ (γX γe κs κl γκl κw γκw κu γκu γr : τ{nat, 1+n}),
           ((⤉ syn_inv n N_ClientSpinlock2 (clientSpinlock2_inv n tid2 ℓL γX γe κs κl γκl κw γκw κu γκu γr))
              ∗ (syn_tgt_interp_as n sndl (fun m => (s_memory_black m)))
              ∗ (⤉ isSpinlock n X γX γe κs ℓL)
              ∗ TID(tid)
              ∗ (⤉ Duty(tid) [(κw, 0, (▿ γκw γκu)); (κl, 0, (▿ γκl γκw))])
              ∗ ◇[κl](2, 1)
              ∗ ◇[κl](2 + ℓL, 5)
              ∗ (⤉ △ γκl (1/2))
              ∗ (⤉ △ γκw (1/2))
  ...
  Proof.
    ...
    red_tl. iIntros (κl). red_tl. iIntros (γκl). red_tl. iIntros (κw). red_tl. iIntros (γκw).
    ...
    iIntros "(#INV_CL & #MEM & #ISL & TID & DUTY & PC & PC2 & LIVE_l & LIVE_w & DELw & PCw)".
    ...
    { iFrame. }
    iApply (wpsim_yieldR_gen_pending with "DUTY [DELw] [PCS]").
    auto.
    { instantiate (2:=[(κw, 0, (▿ γκw γκu)%S)]). instantiate (1:=[(κl, 0, (▿ γκl γκw)%S)]). ss. }
    3:{ iApply pps_cons_fold; iFrame. iApply pps_nil. }
    3:{ iFrame. }
    { econs; ss. }
    lia.
    iEval (simpl). iIntros "DUTY _ DELw PCS". iModIntro. rred2r. iApply wpsim_tauR. rred2r.

    TODO (* Fix Spinlock spec. *)

    iPoseProof ((Spinlock_lock_spec tid n ⊤) $! X γX γe κs ℓL) as "SPEC".
    set (P0 := (△ γκl (1/2))%S : sProp n).
    set (R0 := ((△ γκl (1/2)) ∗ Duty(tid) [(κl, 0, (▿ γκl γκw))])%S : sProp n).
    set (Q0 := ((▿ γκl γκw) ∗ (Duty(tid) [(κw, 0, (▿ γκw γκu))]) ∗ ◇[κw](ℓl, 1))%S : sProp n).
    ...
    { Local Opaque env_live_chain.
      ...
      { (* ELC. *)
        ...
        - iDestruct "CL2" as "(LXw & PTD & %κw & CL2)". iEval (red_tl) in "CL2". iDestruct "CL2" as "[%γκw CL2]".
          iEval (red_tl_all; simpl) in "CL2". iDestruct "CL2" as "(#PROM_w & LIVE_w & #DEAD_l & DUTY2 & DPu & LINKu & LIVE_u & LO_w)".
          iDestruct "LO_w" as "[% LO]". iEval (red_tl; simpl) in "LO". iPoseProof "LO" as "[#LO %LAY]".
          iMod ("INV_CL_CLOSE" with "[LXw PTD LIVE_w DUTY2 DPu LINKu LIVE_u]") as "_".
          { iEval (unfold clientSpinlock2_inv; simpl; red_tl_all; simpl). iRight. iLeft. iFrame.
            iExists _. iEval (red_tl_all). iExists _. iEval (red_tl_all; simpl). iSplitR. iApply "PROM_w". iFrame.
          ...
          iExists κw, x, (((⤉ syn_inv n N_Spinlock (spinlockInv n X γX γe)) ∗ ((⤉ spinlockInv n X γX γe =| S n |={ ⊤ ∖ ↑N_Spinlock, ⊤ }=∗ emp) ∗ ⤉ spinlockInv n X γX γe)) ∗ (⤉ (▿ γκw γκu)))%S.
          ...
          { (* ELC at 0. *)
            ...
            { iEval (simpl; red_tl_all) in "FULF_w".
              iModIntro. iRight. iFrame. iEval (red_tl_all). eauto.
            }
          }
          (* ELC continuation. *)
          iModIntro. iEval (simpl; red_tl; simpl). iIntros "FC [A B]".
          ...
          + iDestruct "CL3" as "(LXw & PTD & % & CL3)".
            iEval (red_tl_all; simpl) in "CL3". iDestruct "CL3" as "(#PROM_u & LIVE_u & #ACTu & #LINK_u & #DEAD_l2 & #DEAD_w2 & PASS)".
            ...
            { iMod (link_amplify with "[PC_u]") as "PC_s".
              ...
              { iEval (unfold clientSpinlock2_inv; simpl; red_tl_all; simpl). do 2 iRight. iLeft. iFrame.
                iExists _. iEval (red_tl_all; simpl). iFrame. iSplit. iApply "PROM_u". iSplit; eauto.
              }
        ...
        - iDestruct "CL3" as "(LXw & PTD & %γκw & CL3)".
          iEval (red_tl_all; simpl) in "CL3". iDestruct "CL3" as "(#PROM_u & LIVE_u & #ACTu & #LINK_u & #DEAD_l & #DEAD_w & PASS)".
          iMod (tpromise_progress with "[FC]") as "[PC_u | #FULF_u]".
          { iFrame. iApply "PROM_u". }
          { iMod (link_amplify with "[PC_u]") as "PC_s".
            { iFrame. iApply "LINK_u". }
      ...
      { (* AU. *)
        ...
        iEval (unfold clientSpinlock2_inv; simpl; red_tl_all; simpl) in "CL".
        iDestruct "CL" as "[CL1 | [CL2 | [CL3 | CL4]]]".
        - iDestruct "CL1" as "(LXw & PTD & #PROM_l & LIVE_l2 & DUTY2 & #PROMu & #LINKu & LIVEu & LO_l)".
          iMod (AuthExcls.b_w_update _ _ _ 1 with "LX LXw") as "[LX LXw]".
          iMod (alloc_obligation ℓl 2) as "(%κw & #LO_w & PC_w & DEL_w)".
          ...
          iPoseProof (unfold_tpromise with "PROM_l") as "[_ #ACTl]".
          iMod (duty_fulfill with "[DUTY]") as "DUTY".
          { iFrame. iSplitL; [|auto]. iEval (simpl; red_tl_all). iExists _. iEval (red_tl_all). eauto. }
          iPoseProof (pc_split _ _ 1 1 with "PC_w") as "[PC_w PC_w2]".
          iMod (pc_drop _ 1 _ _ 1 _ with "PC_w2") as "PC_w2".
          Unshelve. 1,3: lia.
          iEval (rewrite <- (Qp.div_2 1)) in "DEL_w".
          iPoseProof (pending_split with "DEL_w") as "[DEL_w1 DEL_w]".
          iMod (duty_add with "[DUTY PC_w2 DEL_w1] []") as "DUTY".
          { iSplitL "DUTY". iFrame. subst ℓl. iFrame. }
          { instantiate (1:= ((▿ γκw γκu)%S : sProp n)). iModIntro. iEval (simpl; red_tl_all).
            iIntros "#DEAD_w". iModIntro. auto.
          }
          iPoseProof (duty_delayed_tpromise with "DUTY") as "#DPROM_w".
          { ss. eauto. }
          iMod (activate_tpromise with "DPROM_w DEL_w") as "[#PROM_w _]".
          iMod ("INV_CL_CLOSE" with "[LXw PTD LIVE_w2 DUTY2 LIVEu LO_l]") as "_".
          { iEval (unfold clientSpinlock2_inv; simpl; red_tl_all; simpl). iRight. iLeft. iFrame.
            iExists κw. iEval (red_tl_all). iExists γκw. iEval (red_tl_all; simpl). iFrame.
            do 4 (iSplit; [auto|]). iExists ℓl. iEval (red_tl). subst. iSplit; auto.
          }
        ...
        - iExFalso. iDestruct "CL4" as "(_ & _ & % & CL4)".
    ...
    { iDestruct "CL2" as "(LXw & PTD & %κw0 & CL2)". iEval (red_tl) in "CL2". iDestruct "CL2" as "[%γκw0 CL2]".
      iEval (red_tl_all; simpl) in "CL2". iDestruct "CL2" as "(#PROM_w & LIVE_w2 & #DEAD_l2 & DUTY2 & #PROM_u & #LINK_u & LIVEu & LO_w)".
      iDestruct "LO_w" as "[% LO]". iEval (red_tl; simpl) in "LO". iPoseProof "LO" as "[#LO %LAY]".
      ...
      iMod (activate_tpromise with "PROM_u DUTY2") as "[#PROMu #ACTu]".
      ...
      iMod ("INV_CL_CLOSE" with "[LXw PTD LIVEu EX]") as "_".
      { iEval (unfold clientSpinlock2_inv; simpl; red_tl_all; simpl). do 2 iRight. iLeft. iFrame.
        iExists γκw. iEval (red_tl_all; simpl).
        iSplitR. auto. iSplitL "LIVEu". iFrame. do 4 (iSplitR; [auto|]). iLeft. iFrame.
      }
      iMod (duty_fulfill with "[DUTY]") as "DUTY".
      { iFrame. TODO iEval (simpl; red_tl). iExists _. iEval (red_tl_all). eauto. }
