Definition a {n : nat} := n + n. Check a : nat + 3.
...
From Fairness Require Import Client05 AuthExclsRA ExclsRA OneShotsRA.
...
Module Client05Correct.
  ...
  Local Instance HasAuthExcls : GRA.inG (AuthExcls.t nat) Γ := (@GRA.InG _ Γ 11 (@eq_refl _ _)).
  Local Instance HasOneShots : GRA.inG (OneShots.t unit) Γ := (@GRA.InG _ Γ 12 (@eq_refl _ _)).
  Local Instance HasAuthExcls2 : GRA.inG (AuthExcls.t (nat * nat)) Γ := (@GRA.InG _ Γ 13 (@eq_refl _ _)).
  Local Instance HasExcls : GRA.inG (Excls.t unit) Γ := (@GRA.InG _ Γ 14 (@eq_refl _ _)).
  ...
  Local Instance _IINVSETRA : GRA.inG (IInvSetRA sProp) Σ := (@GRA.InG _ Σ 16 (@eq_refl _ _)).
  Local Instance _ARROWRA : GRA.inG (ArrowRA id_tgt_type (Vars:=sProp)) Σ := (@GRA.InG _ Σ 17 (@eq_refl _ _)).
  Local Instance _SHAREDUTY : GRA.inG (ShareDutyRA id_tgt_type (Vars:=sProp)) Σ := (@GRA.InG _ Σ 18 (@eq_refl _ _)).
  ...
  Local Definition init_res :=
    (GRA.embed (memory_init_resource Client05.gvs))
      ⋅ (GRA.embed (AuthExcls.rest_ra (gt_dec 0) 0))
  ...
  Proof.
    ...
    { simpl. iFrame. iDestruct "A" as "[[[A B] C] D]". iSplitL "A"; iFrame. }
    iEval (rewrite red_syn_fairI) in "RES". simpl. iMod "RES".
    iDestruct "RES" as "(% & % & % & % & % & #INV1 & TGTST & #SPIN & #SPIN1 & TID1 & TID2)".
    iEval (rewrite red_syn_tgt_interp_as) in "TGTST". iPoseProof "TGTST" as "#TGTST".

    iModIntro. unfold natmap_prop_sum. ss.
    iSplitL "TID1".
    { iPoseProof (Client05_thread1_spec) as "RES".
      iEval (red_tl) in "RES". iSpecialize ("RES" $! γw).
      iEval (red_tl) in "RES". iSpecialize ("RES" $! kw).
      iEval (red_tl) in "RES". iSpecialize ("RES" $! γL).
      iEval (red_tl) in "RES". iSpecialize ("RES" $! γe).
      iEval (red_tl) in "RES". iSpecialize ("RES" $! γs). simpl. red_tl_all.
      iEval (rewrite red_syn_wpsim) in "RES". iApply ("RES" with "[-]").
      red_tl_all. iFrame.
      rewrite red_syn_inv. rewrite red_syn_tgt_interp_as. simpl. do 4 (iSplit; [done | ]).
      iDestruct "TID1" as "(TID1 & DUTY & PC)"; iFrame.
    }
    iSplitL. 2: done.
    { iPoseProof Client05_thread2_spec as "RES".
      iEval (red_tl) in "RES". iSpecialize ("RES" $! γw).
      iEval (red_tl) in "RES". iSpecialize ("RES" $! kw).
      iEval (red_tl) in "RES". iSpecialize ("RES" $! γL).
      iEval (red_tl) in "RES". iSpecialize ("RES" $! γe).
      iEval (red_tl) in "RES". iSpecialize ("RES" $! γs). simpl. red_tl_all.
      iEval (rewrite red_syn_wpsim) in "RES". iApply ("RES" with "[-]").
      red_tl_all. iDestruct "TID2" as "(A1 & A2 & A3)". iFrame.
      rewrite red_syn_inv. rewrite red_syn_tgt_interp_as. do 3 (iSplit; [done | ]). done.
    }
  Qed.
