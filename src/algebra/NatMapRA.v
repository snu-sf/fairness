  ...
  Section MAP.
    ...
    Proof.
      ...
      - intros x. rewrite /cmra.core pcore_unfold op_unfold /core /add /=.
        des_ifs. f_equal. rewrite union_empty. auto.
      - intros ???.
        rewrite /cmra.core pcore_unfold /core /add /=.
...
From Fairness Require Import PCM IPM ListIProp IUpd.
...
Section UPDNATMAP.
  ...
  Proof.
    iIntros "B W".
    iCombine "B W" gives %(_&Ha&_)%auth_both_dfrac_valid_discrete.
    by apply NatMapRA.extends_singleton_iff in Ha.
  Qed.
  ...
  Lemma NatMapRA_singleton_unique k0 k1 a0 a1
    ...
    (OwnM (◯ singleton k0 a0))
      ...
      (⌜k0 <> k1⌝).
  Proof.
    iIntros "W0 W1". by iCombine "W0 W1" gives %?%auth_frag_valid%singleton_unique.
  Qed.
