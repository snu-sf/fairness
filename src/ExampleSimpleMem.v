...
From Fairness Require Export ITreeLib WFLib FairBeh NatStructs Mod pind OpenMod SCM.

Set Implicit Arguments.


Definition call {R} {ID E} `{@eventE ID -< E} `{OpenMod.callE -< E} {A} (fn: string) (arg: A): itree E R :=
  r <- trigger (OpenMod.Call fn arg↑);;
  unwrap (r↓)
.

Definition callR R {ID E} `{@eventE ID -< E} `{OpenMod.callE -< E} {A} (fn: string) (arg: A): itree E R :=
  r <- trigger (OpenMod.Call fn arg↑);;
  unwrap (r↓)
.
...
Section MOD.
  ...
  Definition example_fun:
    ...
    fun _ =>
      b <- call "cas" (loc_l, SCMem.val_nat 0, SCMem.val_nat 1);;
      if (b: bool) then
        call "store" (loc_f, SCMem.val_nat 1)
      else
        ITree.iter
          (fun _ =>
             f <- call "load" loc_f;;
             b <- call "compare" (f: SCMem.val, SCMem.val_nat 1);;
...
Section SIM.
              ...
              (OwnM (Auth.white (Excl.just im_tgt: @Excl.t (imap thread_id nat_wf)): identThsRA))
                **
                (match w with
                 ...
                 | W_own th k o i => points_to loc_l (SCMem.val_nat 1) ** points_to loc_f (SCMem.val_nat 0) ** ⌜im_tgt th <= i⌝ ** own_thread th ** monoWhite k ge_PreOrder o
  ...
  Lemma close_itree_trigger_call2
        ...
        fn args ktr
    :
    @OMod.close_itree omd md R (trigger (OpenMod.Call fn args) >>= ktr) = UB.
      (* match (md.(Mod.funs) fn) with *)
      (* | Some body => *)
      (*     trigger (Yield);;; rv <- OMod.embed_itree omd md (body args);; tau;; OMod.close_itree omd md (ktr rv) *)
  ...
  Proof.
    ...
    { i. exists (S o0). ss. }
    { cut (forall tid,
              own_thread tid ⊢ stsim I tid itop5 itop5 (fun r_src r_tgt => I ** own_thread tid ** ⌜r_src = r_tgt⌝) (example_spec_fun tt) (OMod.close_itree (example_omod) (SCMem.mod [1; 1]) (example_fun tt))).
      { admit. }
      i. iIntros "TID". ss. unfold example_spec_fun, example_fun.
