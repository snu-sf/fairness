...
Class Monoid {M : Type} (o : M → M → M) := {
  monoid_unit : M;
  monoid_assoc : Assoc (=) o;
  monoid_comm : Comm (=) o;
  monoid_left_id : LeftId (=) monoid_unit o;
}.
Lemma monoid_proper {M : Type} {o : M → M → M} `{!Monoid o} : Proper ((=) ==> (=) ==> (=)) o.
Proof. intros ??????. subst. done. Qed.
Lemma monoid_right_id {M : Type} {o : M → M → M} `{!Monoid o} : RightId (=) monoid_unit o.
...
Class WeakMonoidHomomorphism {M1 M2 : Type}
...
Class MonoidHomomorphism {M1 M2 : Type}
    (o1 : M1 → M1 → M1) (o2 : M2 → M2 → M2) `{!Monoid o1, !Monoid o2}
    (R : relation M2) (f : M1 → M2) := {
