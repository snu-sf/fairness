From Fairness Require Import Red.
From sflib Require Import sflib.
Require Import String.
Require Import Program.

Definition red_class: Type := string.
...
Arguments mk_red_db _ [_] _ [_] _ _.
Arguments red_lemma [_ _ _] _.
Arguments red_next [_ _ _] _.

Definition itree_class: red_class := "itree".

Ltac red_tac c f :=
  match goal with
  | [ |- ?term = _ ] =>
      let tc := fresh "_TC_" in
      unshelve evar (tc: @red_db c _ term); [typeclasses eauto; fail|];
...
Module TUTORIAL.
  Section FOO.
    ...
    Instance foo_red3_hint: red_db itree_class x :=
      mk_red_db _ _ foo_red3 _continue.
    Instance foo_red4_hint: red_db itree_class y :=
      mk_red_db _ _ foo_red4 _continue.
    ...
    Proof.
      ...
      (* time (prw red_B 2 2 1 0). *)

      time (prw ltac:(red_tac itree_class) 2 2 1 0).
