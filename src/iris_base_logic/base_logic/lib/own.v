(* Similar to [iprop.v], the original code is maintained as much as possible.

   - The original definitions and unused lemmas are commented out.
   - For the proofs, its mostly removing stuff about cmra morphisms, so that is simply done.
*)

From iris.algebra Require Import functions gmap proofmode_classes.
From iris.proofmode Require Import proofmode.
From Fairness.base_logic.lib Require Export iprop.
...
Global Arguments inG_id {_ _} _.
Global Arguments inG_apply {_ _} _.
...
Lemma subG_inG Σ (F : gFunctor) : subG F Σ → inG Σ F.
Proof. move=> /(_ 0%fin) /= [j ->]. by exists j. Qed.
...
(* Add a proper instance to help inference. *)
Local Instance iRes_singleton_proper γ : Proper ((≡)==>(≡)) (@iRes_singleton Σ A _ γ) := ne_proper _.
Local Lemma iRes_singleton_validI γ a : ✓ (iRes_singleton γ a) ⊢@{iPropI Σ} ✓ a.
Proof.
  (* Quite different proof due to requiring discrete cmra. *)
  rewrite /iRes_singleton !discrete_valid. iPureIntro.
  intros Ha. specialize (Ha (inG_id i)).
  rewrite discrete_fun_lookup_singleton singleton_valid in Ha.
  by eapply cmra_transport_valid.
Qed.
...
Proof.
  ...
  intros x Hx. assert (cmra_transport inG_prf a ≡ x) as Ha.
  { apply (discrete_0 _). by rewrite -Hx. }
  by rewrite Ha.
Qed.
...
(* Add a proper instance to help some inference. For example in own_mono *)
Global Instance own_proper γ : Proper ((≡) ==> (⊣⊢)) (@own Σ A _ γ) := ne_proper _.
...
Proof.
  ...
  - rewrite /bi_emp_valid (ownM_unit emp).
    apply bupd_ownM_updateP, (discrete_fun_singleton_updateP_empty _ (λ m, ∃ γ,
      m = {[ γ := (cmra_transport inG_prf (f γ)) ]} ∧ P γ));
      [apply _| |naive_solver].
    apply (alloc_updateP_strong_dep _ P _ (λ γ,
      (cmra_transport inG_prf (f γ)))); [done| |naive_solver].
    intros γ _ ?.
    by apply cmra_transport_valid, Hf.
...
Proof.
  ...
  - apply bupd_ownM_updateP, (discrete_fun_singleton_updateP _ (λ m, ∃ x,
      m = {[ γ := x ]} ∧ ∃ a',
      x = cmra_transport inG_prf a' ∧ P a')); [apply _| |naive_solver].
    apply singleton_updateP'.
...
Proof.
  ...
  apply (alloc_unit_singleton_update (cmra_transport inG_prf ε)).
