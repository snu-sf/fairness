...
Section PROOF.
  ...
  Lemma lsim_implies_ksim
        ...
        gps gpt
        (LSIM: forall im_tgt, exists im_src r_shared (os: (nm_wf_stt R0 R1).(T)) rs_ctx,
            (<<RSWF: Th.find tid rs_ctx = None>>) /\
              (<<OSWF: (forall tid', Th.In tid' ths_src -> Th.In tid' os) /\ (Th.In tid os)>>) /\
              (<<LSIM:
                forall im_tgt0
                ...
                exists im_src0,
                  (fair_update im_src im_src0 (sum_fmap_l (tids_fmap tid (NatSet.add tid (key_set ths_src))))) /\
                    (lsim (wf_stt) I tid (local_RR I RR tid)
                          gps gpt (sum_of_resources rs_ctx) (get_o tid os, src) tgt
  ...
  Proof.
    ...
    intros THSRC THTGT WF sf src0 st_src st_tgt r_shared RSWF OSWF0 OSWF1 im_tgt im_src ELRR Eshr Ers Esrc.
    ...
    { clear IH rr. unfold local_RR in LSIM0. des. clarify.
      ...
      { destruct (Th.is_empty ths_tgt) eqn:EMPT.
        ...
        { pfold. eapply pind10_fold. econs 2; eauto.
          ...
          { instantiate (1:=Th.add tid o0 os). ss. econs. all: eauto.
            - apply nm_find_add_eq.
            - eapply NatMapP.F.in_find_iff in OSWF1. destruct (NatMap.find tid os) eqn:FINDOS; ss.
              unfold get_o. erewrite nm_find_some_tfind; eauto.
            - i. eapply nm_find_add_neq; auto.
          }
          ...
          - i; clarify.
            ...
            { i. hexploit LOCAL. eauto. eauto.
              ...
              - intro SYNC. eapply H3 in SYNC. ii. unfold local_sim_sync in SYNC.
                ...
                specialize (SYNC _ _ _ _ _ _ _ INV0 URAWF fs ft _ FAIR0).
                replace o with (get_o tid1 os); auto.
                rewrite ORD. unfold get_o. unfold tfind.
              ...
              - intro PICK. eapply H4 in PICK. ii. unfold local_sim_pick in PICK.
                ...
                specialize (PICK _ _ _ _ _ _ _ INV0 URAWF _ FAIR0).
                replace o with (get_o tid1 os); auto.
                rewrite ORD. unfold get_o. unfold tfind.
                rewrite nm_find_add_neq; auto.
            }
            ...
            { move OSWF0 after NEQ. i. eapply NatMapP.F.add_in_iff. destruct (tid_dec tid' tid).
              { left; auto. }
              right. apply OSWF0. eapply nm_pop_res_is_add_eq in H. rewrite H.
              eapply NatMapP.F.add_in_iff. right; auto.
            }
            { apply NatMapP.F.add_in_iff. right. eapply OSWF0. eapply NatMapP.F.in_find_iff.
              ii. apply nm_pop_find_some in H. rewrite H in H3. ss.
            }
            assert (PROJS: NatSet.remove tid (NatSet.add tid (key_set ths_src)) = NatSet.add tid0 (key_set ths_src0)).
            { eapply proj_aux; eauto. }
            rewrite <- PROJS.
            replace (get_o tid0 (Th.add tid o0 os)) with (get_o tid0 os).
            eapply H2; eauto.
            ...
            { unfold get_o. unfold tfind. rewrite nm_find_add_neq; auto. }
          ...
          - i. clarify.
            ...
            { i. hexploit LOCAL. 1,2:eauto.
              ...
              - intro SYNC. eapply H4 in SYNC. clear H4 H5. ii. unfold local_sim_sync in SYNC.
                ...
                + destruct (tid_dec tid0 tid1); auto. clarify.
                  ...
                  hexploit nm_pop_res_is_rm_eq. eapply H0. i. clarify.
                  rewrite nm_find_rm_eq in LTGT. ss.
                + rewrite ORD. unfold get_o. unfold tfind. rewrite nm_find_add_neq; auto.
            ...
            { apply NatMapP.F.add_in_iff. right. eapply OSWF0. eapply NatMapP.F.in_find_iff.
              ii. apply nm_pop_find_some in H. rewrite H in H4. ss.
            }
            rewrite <- PROJS. replace (get_o tid0 (Th.add tid o0 os)) with (get_o tid0 os).
            eauto.
            { unfold get_o. unfold tfind. rewrite nm_find_add_neq; auto. }
        }
      }
    ...
    { clear IH rr. clarify. rewrite ! bind_trigger.
      ...
      - i; clarify.
        ...
        { rename tid0 into tid.
          ...
          { i. hexploit LOCAL. 1,2:eauto. 1,2: eauto. i; des. split.
            - intro SYNC. eapply H2 in SYNC. clear H2 H3. ii. unfold local_sim_sync in SYNC.
              replace o with (get_o tid0 os).
              eapply SYNC; eauto.
              ...
              + rewrite ORD. destruct (tid_dec tid tid0); clarify.
                unfold get_o. unfold tfind. rewrite nm_find_add_neq; auto.
          ...
          { move OSWF0 after STUTTER. i. eapply NatMapP.F.add_in_iff. destruct (tid_dec tid' tid).
            { left; auto. }
            right. apply OSWF0. auto.
          }
          { apply NatMapP.F.add_in_iff. left; auto. }
          ...
          replace (sum_of_resources (snd (get_resource tid (NatMap.add tid r_own rs_ctx)))) with
            (sum_of_resources rs_ctx).
          replace (get_o tid (Th.add tid o1 os)) with o1.
          auto.
          - unfold get_o. erewrite nm_find_some_tfind; eauto. apply nm_find_add_eq.
        ...
        { i. destruct (tid_dec tid tid1) eqn:TID2; subst.
          { rename tid1 into tid.
            ...
            { rewrite bind_trigger. f_equal. f_equal. extensionality x. destruct x. ss. }
            rewrite H3. eapply lsim_set_prog.
            replace (get_o tid (Th.add tid o1 os)) with o1; auto.
            { unfold get_o. unfold tfind. rewrite nm_find_add_eq; auto. }
          }

          { hexploit LOCAL. 1,2:eauto.
            eapply find_some_neq_aux; eauto. eapply find_some_neq_aux; eauto.
            i; des. split.
            - intro SYNC. eapply H2 in SYNC. clear H2 H3. ii. unfold local_sim_sync in SYNC.
              replace (get_o tid1 (Th.add tid o1 os)) with (get_o tid1 os).
              eapply SYNC; eauto.
        ...
        { move OSWF0 after TID. i. eapply NatMapP.F.add_in_iff. destruct (tid_dec tid' tid).
          { left; auto. }
          right. apply OSWF0. eapply nm_pop_res_is_rm_eq in H. rewrite <- H in H2.
          eapply NatMapP.F.remove_in_iff in H2. des.
          eapply NatMapP.F.add_in_iff in H3. des; clarify.
        }
        { apply NatMapP.F.add_in_iff. right. eapply OSWF0. eapply NatMapP.F.in_find_iff.
          ii. apply nm_pop_find_some in H.
          rewrite nm_find_add_neq in H; auto. rewrite H in H2; ss.
        }
        ...
        rewrite <- PROJS. replace (get_o tid0 (Th.add tid o1 os)) with (get_o tid0 os).
      ...
      - i; clarify. destruct (tid_dec tid tid0) eqn:TID1.
        ...
        { i. destruct (tid_dec tid tid1) eqn:TID2; subst.
          { rename tid1 into tid.
            ...
            { rewrite bind_trigger. f_equal. f_equal. extensionality x. destruct x. ss. }
            rewrite H5. eapply lsim_set_prog.
            replace (get_o tid (Th.add tid o1 os)) with o1; auto.
            { unfold get_o. unfold tfind. rewrite nm_find_add_eq; auto. }
          }
          ...
          { hexploit LOCAL. 1,2:eauto.
            ...
            - intro SYNC. eapply H4 in SYNC. clear H4 H5. ii. unfold local_sim_sync in SYNC.
              ...
              + rewrite get_resource_rs_neq in VALID0. rewrite get_resource_add_neq_fst in VALID0; auto.
                ...
                hexploit nm_pop_res_is_rm_eq; eauto. i. rewrite <- H in LTGT.
                rewrite nm_find_rm_eq in LTGT. ss.
              + unfold get_o, tfind. rewrite nm_find_add_neq; auto.
            - intro PICK. eapply H5 in PICK. clear H4 H5. ii. unfold local_sim_pick in PICK.
              replace (get_o tid1 (Th.add tid o1 os)) with (get_o tid1 os).
              eapply PICK; eauto.
              + rewrite get_resource_rs_neq in VALID0. rewrite get_resource_add_neq_fst in VALID0; auto.
        ...
        { move OSWF0 after H3. i. eapply NatMapP.F.add_in_iff. destruct (tid_dec tid' tid).
          { left; auto. }
          right. apply OSWF0. eapply nm_pop_res_is_rm_eq in H. rewrite <- H in H4.
          eapply NatMapP.F.remove_in_iff in H4. des.
          eapply NatMapP.F.add_in_iff in H5. des; clarify.
        }
        { apply NatMapP.F.add_in_iff. right. eapply OSWF0. eapply NatMapP.F.in_find_iff.
          ii. apply nm_pop_find_some in H.
          rewrite nm_find_add_neq in H; auto. rewrite H in H4; ss.
        }
        rewrite <- PROJS. eapply lsim_set_prog.
        replace (get_o tid0 (Th.add tid o1 os)) with (get_o tid0 os); auto.
        { unfold get_o. unfold tfind. rewrite nm_find_add_neq; auto. }
    }
